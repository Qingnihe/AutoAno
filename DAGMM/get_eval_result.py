from dagmm.evaluate import bf_search
import numpy as np
import pandas as pd
...
def get_bf_by_machine_threhold(calc_latency):
    ...
    # for machine_id in tqdm(range(1,26+1)):
...
def read_bf_all_result(calc_latency):
    ...
    r = tp / (tp+fn)
    f1 = 2*p*r / (p+r)
    all_res_file = open(exp_dir/f'evaluation_result/all_res.txt', mode='a')
    print(f"all result get_bf f1: {round(f1, 4)} p:{round(p, 4)} r:{round(r, 4)}")
    print(f"all result get_bf f1: {round(f1, 4)} p:{round(p, 4)} r:{round(r, 4)}", file=all_res_file)
    return round(f1, 4)

def get_exp_result():
    (project_path/'result/evaluation_result').mkdir(exist_ok=True, parents=True)
