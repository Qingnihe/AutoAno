...
#################################################################
        ...
        threshold = get_best_f1(np.array([-x for x in test_score]), np.array(label[i]))
        with open(os.path.join(config.result_dir, f'{i}/{config.threshold_filename}'), 'w') as file:
            file.write(str(threshold))

        ws = config.window_size[config.dataset]
        stride = 1
        wn = len(label[i])

        t_per_win = (end_predict - start_predict) / ((wn - ws) / stride + 1)
        print(config.dataset)
        print('train time:', end_fit - start_fit )

        print('test time per win:', t_per_win * 1000,'ms')
