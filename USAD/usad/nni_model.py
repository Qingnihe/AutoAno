...
class USAD:
    ...
    def compose_loss(self, w, w_G, w_G_D, w_D, epoch, loss_type='train'):
        index_loss_weight = [1 for _ in range(self._x_dims)]
    ...
    def fit(self, values, model_save_dir: pathlib.Path, local_epoch, valid_portion=0.3):
        ...
        # 基于对抗学习两个loss的目标是相反的，理想状态下loss的差值是越来越大，并趋于稳定的，因此选择差值最大的step对应的model
        ...
        for epoch in range(1, local_epoch+1):
            for i, w in enumerate(train_data):
                ...
                if self._step > 0 and self._step % self._valid_step_freq == 0:
                    nni.report_intermediate_result(self.loss['AE_G']['train'][-1]-self.loss['AE_D']['train'][-1])
        ...
        nni.report_final_result(self.loss['AE_G']['train'][-1]-self.loss['AE_D']['train'][-1])
        self.valid_log(valid_data, model_save_dir, log_file, epoch)
    ...
    def predict(self, values, alpha=0.5, beta=0.5, on_dim=False):
        ...
        with torch.no_grad():
            for w in test_sliding_window:
                ...
                batch_scores = alpha * self.loss_func(w_G, w) + beta * self.loss_func(w_G_D, w)
