...
from models.evaluate import mse_nf
...
class TrainGDN:
    def __init__(self, train_config, env_config):
        ...
        self.feature_map = list(range(train_config["feature_dim"]))
    ...
    def fit(self, save_path: pathlib.Path, train_values):
        ...
        for i_epoch in range(1, epochs+1):
            ...
            if i_epoch % global_valid_epoch_freq == 0:
                nni.report_intermediate_result(acu_loss)
                self.valid_log(save_path, i_epoch, epochs, acu_loss, log_file, dataloader, min_loss)
                # autoKad
                # mse_nf_loss = mse_nf(labels.detach().cpu().numpy(), out.detach().cpu().numpy())
                # nni.report_intermediate_result(mse_nf_loss)
                # print(f'epoch ({i_epoch} / {epochs}) mse_nf_loss:{mse_nf_loss}')
                # print(f'epoch ({i_epoch} / {epochs}) mse_nf_loss:{mse_nf_loss}', file=log_file)
                
                # f1
                # self.valid_log(save_path, i_epoch, epochs, acu_loss, log_file, dataloader, min_loss)
                # self.restore(save_path)
                # nni_f1_time = open(exp_dir/'nni_f1_time.txt','a')
                # test_start_time = time.time()
                # avg_loss, test_predicted_list, test_ground_list = self.predict(test_values=test_values)
                
                # score = get_err_scores(test_predicted_list, test_ground_list)
                # result_path = exp_dir / f"result/nni_f1"
                ...
                # nni_f1_time.write('epoch:{} f1 test time: {}\n'.format(i_epoch, test_time))
                # nni_f1_time.close()
        ...
        # autoKad
        # mse_nf_loss = mse_nf(labels.detach().cpu().numpy(), out.detach().cpu().numpy())
        # nni.report_final_result(mse_nf_loss)
        # f1
        # self.valid_log(save_path, i_epoch, epochs, acu_loss, log_file, dataloader, min_loss)
        # self.restore(save_path)
        # nni_f1_time = open(exp_dir/'nni_f1_time.txt','a')
        ...
        nni.report_final_result(acu_loss)        
        self.valid_log(save_path, i_epoch, epochs, acu_loss, log_file, dataloader, min_loss)
