...
def test(i, config: Config, train_data_item, test_data_item):
    total_train_time = 0
    ...
    # model.save("/home/sunyongqian/chenshiqi/SDFVAE_pytorch/out_test/model2.pkl")
...
def by_test():
    ...
    for i,(tr,te,lab) in enumerate(zip(train_data,test_data,label)):
        ...
        # print(feature_dim)
        # train_data_item, test_data_item = get_data_by_index(dataset_type, cluster['center'], training_period)
        train_time=test(i, config, tr.T, te.T)
        print(i,train_time,'s')
        total_train_time+=train_time
        # break
